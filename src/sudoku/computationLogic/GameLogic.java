...
public class GameLogic {
    ...
    private static boolean squaresAreInvalid(int[][] grid) {
        if (rowSquaresIsInvalid(Rows.TOP, grid)) return true;

        if (rowSquaresIsInvalid(Rows.MIDDLE, grid)) return true;

        if (rowSquaresIsInvalid(Rows.BOTTOM, grid)) return true;

        return false;
    }

    private static boolean rowSquaresIsInvalid(Rows value, int[][] grid) {
        switch(value) {
            case TOP:
